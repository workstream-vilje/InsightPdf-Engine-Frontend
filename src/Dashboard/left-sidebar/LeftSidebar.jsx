...
const Section = ({ title, icon: Icon, badge, children, defaultOpen = false, index, isCollapsed, sticky = false }) => {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div
      className={classNames(styles.section, styles[`sectionStage${index}`], {
        [styles.sectionCollapsed]: isCollapsed,
        [styles.sectionSticky]: sticky && !isCollapsed,
      })}
    >
      <button
...
const ConfigPanel = ({ isCollapsed, setIsCollapsed }) => {
  ...
  // Selection states
  const [textProcessing, setTextProcessing] = useState([textProcessingOptions[0]]);
  ...
  return (
    <div className={classNames(styles.configPanel, { [styles.collapsed]: isCollapsed })}>
      ...
      <ScrollArea className={styles.scrollArea}>
        <Section title="Chunk Length" icon={Ruler} defaultOpen={true} index={0} isCollapsed={isCollapsed} sticky>
        ...
        <Section title="Text Processing" icon={FileText} defaultOpen={false} index={1} isCollapsed={isCollapsed}>
          <Field label="Splitter Strategy (Multi-select)">
            <Select
              isMulti
